...
def merge_into_row_with_intensity(ele, pred, pred_intensity):
    def preprocess_depth(x):
        y = np.squeeze(x.data.cpu().numpy())
        return depth_colorize(y)
    # if is gray, transforms to rgb
    ...
    if 'd' in ele:
        img_list.append(preprocess_depth(ele['d'][0,...]))
    if 'intensity' in ele:
        img_list.append(preprocess_depth(ele['intensity'][0,...]))
    img_list.append(preprocess_depth(pred[0,...]))
    img_list.append(preprocess_depth(pred_intensity[0,...]))
    if 'gt' in ele:
        img_list.append(preprocess_depth(ele['gt'][0,...]))
    if 'gt_intensity' in ele:
        img_list.append(preprocess_depth(ele['gt_intensity'][0,...]))

    img_merge = np.hstack(img_list)
    return img_merge.astype('uint8')

def merge_into_row_with_intensity2(ele, pred, pred_pure, pred_intensity):
    def preprocess_depth(x):
        y = np.squeeze(x.data.cpu().numpy())
        return depth_colorize(y)
    # if is gray, transforms to rgb
    ...
    img_list.append(preprocess_depth(pred[0,...]))
    img_list.append(preprocess_depth(pred_pure[0,...]))
    img_list.append(preprocess_depth(pred_intensity[0,...]))
    if 'gt' in ele:
        img_list.append(preprocess_depth(ele['gt'][0,...]))
    if 'gt_intensity_pure' in ele:
        img_list.append(preprocess_depth(ele['gt_intensity_pure'][0,...]))
    if 'gt_intensity' in ele:
        img_list.append(preprocess_depth(ele['gt_intensity'][0,...]))
